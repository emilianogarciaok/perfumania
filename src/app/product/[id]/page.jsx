    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-white">
            <div className="text-center">
                <h2 className="text-2xl font-serif mb-4 text-gray-800">Perfume no encontrado</h2>
                <BackButton /> 
            </div>
        </div>
    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <div className="flex flex-col h-full pt-2">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4">
                ...
                {etiquetas.map(tag => (
                    <span key={tag} className={`text-[10px] px-2 py-1 rounded-full border uppercase font-semibold ${TAG_COLORS[tag] || TAG_COLORS.Default}`}>
                        {tag}
                    </span>
                ))}
            </div>
            ...
            {perfume.inspiracion && (
                <div className="flex items-center gap-2 mb-6 text-gray-500 text-sm bg-gray-50 w-fit px-3 py-1.5 rounded-lg border border-gray-100">
                    ...
                    <span className="font-medium text-gray-900 italic">{perfume.inspiracion}</span>
                </div>
            )}
            ...
            <div className="mb-8 border-b border-gray-100 pb-6">
                {tieneStock ? (
                     <div className="flex items-end gap-2">
                        <span className="text-4xl font-medium text-gray-900">{precioFormateado}</span>
                        <span className="text-sm text-gray-500 mb-2 font-medium">ARS</span>
                     </div>
                ) : (
                    <span className="text-2xl font-bold text-red-400">No Disponible Temporalmente</span>
                )}
            </div>
