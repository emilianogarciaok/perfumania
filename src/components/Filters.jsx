...
const olfactoryOptions = [
  { value: "Fresco", label: "🌊 Fresco", color: "bg-blue-100 text-blue-800 border-blue-200 hover:bg-blue-200" },
  { value: "Dulce", label: "🍬 Dulce", color: "bg-pink-100 text-pink-800 border-pink-200 hover:bg-pink-200" },
  { value: "Intenso", label: "🔥 Intenso", color: "bg-red-100 text-red-800 border-red-200 hover:bg-red-200" },
  { value: "Floral", label: "🌸 Floral", color: "bg-purple-100 text-purple-800 border-purple-200 hover:bg-purple-200" },
  { value: "Versátil", label: "✨ Versátil", color: "bg-teal-100 text-teal-800 border-teal-200 hover:bg-teal-200" },
];
...
export default function Filters() {
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      if (searchTerm.length > 1) {
        const { data } = await supabase.from('perfumes').select('id, nombre, imagen, precio_venta').ilike('nombre', `%${searchTerm}%`).limit(5);
  ...
  const currentSortLabel = sortOptions.find(opt => opt.value === sort)?.label || "Ordenar por...";
    ...
    <div className="flex flex-col gap-6 mb-8 relative z-30">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-100 relative">
          ...
          {showSuggestions && suggestions.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 rounded-xl shadow-xl max-h-80 overflow-y-auto z-50 animate-in fade-in zoom-in-95 duration-200">
              {suggestions.map((perfume) => (
                <Link key={perfume.id} href={`/product/${perfume.id}`} className="flex items-center gap-4 p-3 hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0 group/item">
                   <div className="relative w-12 h-12 shrink-0 bg-gray-50 rounded-md overflow-hidden border border-gray-100 shadow-sm group-hover/item:border-gray-200">
                       {perfume.imagen && <Image src={perfume.imagen} alt={perfume.nombre} fill className="object-cover" />}
                   </div>
                   <div className="flex-1 min-w-0">
                       <p className="text-sm font-semibold text-gray-800 truncate group-hover/item:text-yellow-700">{perfume.nombre}</p>
                   </div>
                </Link>
              ))}
            </div>
        ...
        <div className="relative w-full md:w-64" ref={sortRef}>
          <button onClick={() => setShowSortMenu(!showSortMenu)} className="flex items-center justify-between w-full py-2.5 px-4 border border-gray-200 bg-gray-50 rounded-lg text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 hover:bg-white transition-all">
          ...
          {showSortMenu && (
            <div className="absolute top-full right-0 mt-2 w-full bg-white border border-gray-100 rounded-xl shadow-xl z-50 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
              {sortOptions.map((option) => (
                <div key={option.value} onClick={() => { setSort(option.value); setShowSortMenu(false); }} className={`px-4 py-3 text-sm cursor-pointer transition-colors border-b border-gray-50 last:border-0 ${sort === option.value ? 'bg-yellow-50 text-yellow-800 font-medium' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}>
                  {option.label}
                </div>
              ))}
            </div>
